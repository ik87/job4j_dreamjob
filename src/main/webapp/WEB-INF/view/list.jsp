...
<head>
    ...
    <script>
        ...
        function updateRow(data) {
            ...
            var tr = $("#" + data.userId);
            //create new row
            var row = jsonToRow(data);

            tr.html(row);

            $("#" + data.userId + " td:last-child").on('click', function () {
                remove(data.userId);
            });

            $("#" + data.userId + " td:not(:last-child)").on('click',function () {
                modalProfileShow(data.userId);
            });
        }

        function addRow(data) {
            var row = jsonToRow(data);
            $('tbody').append('<tr id='+ data.userId +'>' + row + '</tr>');

            $('tbody tr:last td:last-child button').on('click', function () {
                remove(data.userId);
            });
        ...
        function jsonToRow(data) {
          var row =
            '<td><img style="height: 50px" src="data:image/jpeg;base64,' + data.photo +
...
<div class="container">
    <div class="row-cols-1">
        ...
        <%--table users--%>
        <div class="col mt-4 table-responsive-sm">
